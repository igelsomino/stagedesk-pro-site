...
const formatScriptTitle = (name) => String(name || 'Copione')
  .replace(/\.md$/i, '')
  .trim()
  .normalize('NFD')
  .replace(/[\u0300-\u036f]/g, '')
...
const isCollectiveCharacter = (value) => new Set(['tutti', 'tutte', 'coro', 'ensemble', 'tutti insieme', 'prologo']).has(characterKey(value))
...
const renderShare = (updateMessage = '', uiState = {}) => {
  ...
  root.querySelector('[data-script-structure-select]')?.addEventListener('change', (event) => {
    ...
    activeStructureIndex = targetIndex
    if (targetIndex >= scriptRenderLimit) {
      scriptRenderLimit = Math.min(targetIndex + 1, scriptItems.length)
      renderShare('', { scrollToStructureIndex: targetIndex, structureIndex: targetIndex })
      return
    }
